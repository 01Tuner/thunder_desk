        ...
        <div id="windows-style-menu" class="windows-menu-container ${contextClass}">
            <div class="container">
                <div class="windows-menu-bar">
                    ...
                    ${menuStructure.map(menu => getMenuItemHTML(menu, isTabApp, isDesktopApp)).join('')}
                </div>
            </div>
        </div>
    `;
...
function getMenuItemHTML(menu, isTabApp, isDesktopApp) {
    const submenuHTML = menu.submenus.map(submenu => {
    ...
    return [
        ...
        },
        {
            ...
            "submenus": [
                ...
                {"label": __("Request for Quotation"), "doctype": "Request for Quotation", "icon": "fa fa-file-o"},
                {"label": __("Purchase Order"), "doctype": "Purchase Order", "icon": "fa fa-file-text"},
                {"label": __("Purchase Invoice"), "doctype": "Purchase Invoice", "icon": "fa fa-dollar"},
                {"label": __("Purchase Receipt"), "doctype": "Purchase Receipt", "icon": "fa fa-inbox"}
            ]
        },
        {
            "name": "Stock",
            ...
            "submenus": [
                ...
                {"label": __("Stock Reconciliation"), "doctype": "Stock Reconciliation", "icon": "fa fa-balance-scale"}
            ]
        },
        {
            "name": "Reports",
...
function setupWindowsMenuHandlers() {
    ...
    $(document).on('mouseenter.windowsMenu', '.windows-dropdown-menu', function(e) {
        e.stopPropagation();
        $(this).addClass('show').siblings('.windows-menu-button').addClass('show').attr('aria-expanded', 'true');
    });
    ...
    $(document).on('mouseleave.windowsMenu', '.windows-menu-item', function() {
        ...
        setTimeout(() => {
            if (!$item.is(':hover')) {
                ...
                $item.find('.windows-menu-button').removeClass('show').attr('aria-expanded', 'false');
            }
        }, 100);
    });
    ...
    $(document).on('click.windowsMenu', '.windows-submenu-item', function(e) {
        ...
        frappe.show_alert({ message: __('Loading {0}...', [$item.find('span').text()]), indicator: 'blue' });
        $('#windows-style-menu').addClass('windows-menu-loading');
    ...
    $(document).on('keydown.windowsMenu', '.windows-menu-button', function(e) {
        const $current = $(this);
        if (e.which === 37) {
            const $prev = $current.closest('.windows-menu-item').prev().find('.windows-menu-button');
            if ($prev.length) $prev.focus();
        } else if (e.which === 39) {
            const $next = $current.closest('.windows-menu-item').next().find('.windows-menu-button');
